...
Peer_wire_client::Peer_wire_client(bencode::Metadata torrent_metadata, util::Download_resources resources, QByteArray id,
					     QByteArray info_sha1_hash)
    : properties_displayer_(torrent_metadata), id_(std::move(id)), info_sha1_hash_(std::move(info_sha1_hash)),
	handshake_msg_(craft_handshake_message()), dl_path_(std::move(resources.dl_path)), torrent_metadata_(std::move(torrent_metadata)),
...
Peer_wire_client::Peer_wire_client(magnet::Metadata torrent_metadata, util::Download_resources resources, QByteArray id)
    : properties_displayer_(torrent_metadata), id_(std::move(id)), info_sha1_hash_(std::move(torrent_metadata.info_hash)),
	handshake_msg_(craft_handshake_message()), dl_path_(std::move(resources.dl_path)), tracker_(resources.tracker) {
...
void Peer_wire_client::configure_default_connections() noexcept {
	...
	connect(tracker_, &Download_tracker::torrent_open_button_clicked, &properties_displayer_,
		  &Torrent_properties_displayer::display_file_bar);
...
std::int32_t Peer_wire_client::piece_size(const std::int32_t piece_idx) const noexcept {
	...
	const auto piece_size = piece_idx == total_piece_cnt_ - 1 && total_byte_cnt_ % torrent_piece_size_
					    ? total_byte_cnt_ % torrent_piece_size_
					    : torrent_piece_size_;
...
bool Peer_wire_client::verify_piece_hash(const QByteArray & received_piece, const std::int32_t piece_idx) const noexcept {
	...
	const QByteArray piece_hash(torrent_metadata_.pieces.substr(static_cast<std::size_t>(beg_hash_idx), sha1_hash_byte_cnt).data(),
					    sha1_hash_byte_cnt);
...
Peer_wire_client::Piece_metadata Peer_wire_client::piece_info(const std::int32_t piece_idx, const std::int32_t offset) const noexcept {
	...
	const auto block_size =
	    offset / max_block_size == block_cnt - 1 && cur_piece_size % max_block_size ? cur_piece_size % max_block_size : max_block_size;
	...
	auto verify_piece = [this](auto verify_piece_callback, const std::int32_t piece_idx) -> void {
		...
		if(piece_idx < total_piece_cnt_) {
			...
			QTimer::singleShot(0, this,
						 [verify_piece_callback, piece_idx] { verify_piece_callback(verify_piece_callback, piece_idx + 1); });
		} else {
...
[[nodiscard]]
QByteArray Peer_wire_client::craft_piece_message(const QByteArray & piece_data, const std::int32_t piece_idx,
								 const std::int32_t piece_offset) noexcept {
	using util::conversion::convert_to_hex;
...
[[nodiscard]]
std::optional<std::pair<QByteArray, QByteArray>> Peer_wire_client::verify_handshake_reply(Tcp_socket * const socket,
															const QByteArray & reply) const noexcept {
...
void Peer_wire_client::send_block_requests(Tcp_socket * const socket, const std::int32_t piece_idx) noexcept {
	...
	auto send_block_request_impl = [this, piece_idx, total_block_cnt, socket](auto send_block_request_callback,
													  const std::int32_t block_idx) -> void {
...

		{
			...
			if(socket->is_pending_request(request_metadata) || socket->rejected_requests.contains(request_metadata) ||
			   socket->request_sent(request_metadata)) {
				return;
			}
		...
		connect(
		    ...
		    [&requested_blocks = requested_blocks, socket, block_idx, dec_connection] {
			    ...
			    if(socket->state() == Tcp_socket::SocketState::ConnectedState && !socket->fast_extension_enabled &&
				 !requested_blocks.empty()) {
				    assert(requested_blocks[block_idx] > 0);
				    --requested_blocks[block_idx];
...
void Peer_wire_client::on_block_request_received(Tcp_socket * const socket, const QByteArray & request) noexcept {
	...
	if(!is_valid_piece_index(requested_piece_idx) || requested_offset + requested_byte_cnt > piece_size(requested_piece_idx) ||
	   !bitfield_[requested_piece_idx]) {
		return send_reject_message();
	}

	if((!socket->is_good_ratio() || (!socket->peer_interested || socket->am_choking)) &&
	   !socket->allowed_fast_set.contains(requested_piece_idx)) {
		return send_reject_message();
	}
	...
	QTimer::singleShot(
	    0, this, [this, socket = QPointer(socket), send_piece, send_reject_message, requested_piece_idx = requested_piece_idx] {
...
std::optional<std::pair<qsizetype, qsizetype>> Peer_wire_client::beginning_file_handle_info(const std::int32_t piece_idx) const noexcept {
	...
	for(qsizetype file_handle_idx = 0, file_size_sum = 0; file_handle_idx < file_handles_.size(); ++file_handle_idx) {
		...
		const auto end_piece_idx =
		    static_cast<std::int32_t>(std::ceil(static_cast<double>(file_size_sum) / static_cast<double>(torrent_piece_size_))) - 1;

		if(end_piece_idx >= piece_idx) {
			const auto prev_offset_delta =
			    file_size_sum_old % torrent_piece_size_ ? torrent_piece_size_ - file_size_sum_old % torrent_piece_size_ : 0;
...
bool Peer_wire_client::write_to_disk(const QByteArray & received_piece, const std::int32_t received_piece_idx) noexcept {
	...
	for(std::int64_t written_byte_cnt = 0, file_handle_idx = beg_file_handle_idx; written_byte_cnt < received_piece.size();
	    ++file_handle_idx) {
...
std::optional<QByteArray> Peer_wire_client::read_from_disk(const std::int32_t requested_piece_idx) noexcept {
	...
	for(qsizetype file_handle_idx = beg_file_handle_idx; resultant_piece.size() < requested_piece_size; ++file_handle_idx) {
		...
		const auto to_read_byte_cnt = std::min(resultant_piece.size() ? file_size(file_handle_idx) : beg_file_byte_cnt,
								   requested_piece_size - resultant_piece.size());
...
bool Peer_wire_client::is_valid_reply(Tcp_socket * const socket, const QByteArray & reply, const Message_Id received_msg_id) noexcept {
	...
	default: {
		...
		if(static_cast<std::int32_t>(received_msg_id) >= static_cast<std::int32_t>(Message_Id::Suggest_Piece) &&
		   !socket->fast_extension_enabled) {
...
QSet<std::int32_t> Peer_wire_client::generate_allowed_fast_set(const std::uint32_t peer_ip, const std::int32_t total_piece_cnt) noexcept {
	...
	while(allowed_fast_set.size() < allowed_fast_set_size) {
		...
		for(std::int32_t offset = 0; allowed_fast_set.size() < allowed_fast_set_size && offset < allowed_fast_set_size; offset += 4) {
			...
			const auto allowed_fast_idx =
			    util::extract_integer<std::uint32_t>(rand_bytes, offset) % static_cast<std::uint32_t>(total_piece_cnt);
...
void Peer_wire_client::on_piece_downloaded(Piece & dled_piece, const std::int32_t dled_piece_idx) noexcept {
	...
	if(verify_piece_hash(dled_piece.data, dled_piece_idx) && write_to_disk(dled_piece.data, dled_piece_idx)) {
		...
		tracker_->set_ratio(
		    session_uled_byte_cnt_ ? static_cast<double>(session_dled_byte_cnt_) / static_cast<double>(session_uled_byte_cnt_) : 0);
...
void Peer_wire_client::on_block_received(Tcp_socket * const socket, const QByteArray & reply) noexcept {
	...
	const util::Packet_metadata received_packet_metadata{received_piece_idx, received_piece_offset,
									     static_cast<std::int32_t>(received_block.size())};
	...
	if(++received_block_cnt == total_block_cnt) {
		...
		QTimer::singleShot(0, this,
					 [this, received_piece_idx] { on_piece_downloaded(pieces_[received_piece_idx], received_piece_idx); });
	}
...
QByteArray Peer_wire_client::craft_generic_message(const util::Packet_metadata packet_metadata) noexcept {
	...
	return msg + convert_to_hex(packet_metadata.piece_idx) + convert_to_hex(packet_metadata.piece_offset) +
		 convert_to_hex(packet_metadata.byte_cnt);
}
...
void Peer_wire_client::communicate_with_peer(Tcp_socket * const socket) {
...

	{
		if(socket->extension_protocol_enabled && received_msg_id == Message_Id::Extended_Protocol) {
			on_extension_message_received(socket,
								reply->sliced(msg_begin_offset) /* skip standard bit. '20' for extension protocol */);
		}
	}
...
void Peer_wire_client::on_extension_handshake_received(Tcp_socket * const socket, const QByteArray & message) {
	...
	if(metadata_size_ < 1) {
		...
		total_metadata_piece_cnt_ =
		    static_cast<std::int64_t>(std::ceil(static_cast<double>(metadata_size_) / static_cast<double>(max_block_size)));
...
[[nodiscard]]
bool Peer_wire_client::validate_metadata_piece_info(const std::int64_t piece_idx,
								    const std::int64_t received_raw_dict_size) const noexcept {
