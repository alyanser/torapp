...
Peer_wire_client::Peer_wire_client(bencode::Metadata torrent_metadata, util::Download_resources resources, QByteArray id, QByteArray info_sha1_hash)
    : properties_displayer_(torrent_metadata), id_(std::move(id)), info_sha1_hash_(std::move(info_sha1_hash)), handshake_msg_(craft_handshake_message()), dl_path_(std::move(resources.dl_path)),
	torrent_metadata_(std::move(torrent_metadata)), tracker_(resources.tracker),
	total_byte_cnt_(torrent_metadata.single_file ? torrent_metadata.single_file_size : torrent_metadata.multiple_files_size), torrent_piece_size_(torrent_metadata.piece_length),
	...
	spare_piece_cnt_(total_piece_cnt_ % 8 ? 8 - total_piece_cnt_ % 8 : 0), average_block_cnt_(static_cast<std::int32_t>(std::ceil(static_cast<double>(torrent_piece_size_) / max_block_size))),
...
Peer_wire_client::Peer_wire_client(magnet::Metadata torrent_metadata, util::Download_resources resources, QByteArray id)
    : properties_displayer_(torrent_metadata), id_(std::move(id)), info_sha1_hash_(std::move(torrent_metadata.info_hash)), handshake_msg_(craft_handshake_message()),
	dl_path_(std::move(resources.dl_path)), tracker_(resources.tracker) {
...
[[nodiscard]] std::int64_t Peer_wire_client::downloaded_byte_count() const noexcept { return dled_byte_cnt_; }

[[nodiscard]] std::int64_t Peer_wire_client::uploaded_byte_count() const noexcept { return uled_byte_cnt_; }

[[nodiscard]] std::int64_t Peer_wire_client::remaining_byte_count() const noexcept { return total_byte_cnt_ - dled_byte_cnt_; }

[[nodiscard]] bool Peer_wire_client::is_valid_piece_index(const std::int32_t piece_idx) const noexcept { return piece_idx >= 0 && piece_idx < total_piece_cnt_; }

[[nodiscard]] qsizetype Peer_wire_client::file_size(const qsizetype file_idx) const noexcept { return static_cast<qsizetype>(torrent_metadata_.file_info[static_cast<std::size_t>(file_idx)].second); }
...
void Peer_wire_client::configure_default_connections() noexcept {
	...
	connect(tracker_, &Download_tracker::move_files_to_trash, this,
		  [&file_handles_ = file_handles_]() { std::for_each(file_handles_.cbegin(), file_handles_.cend(), [](const auto file_info) { file_info.first->moveToTrash(); }); });

	connect(tracker_, &Download_tracker::delete_files_permanently, this,
		  [&file_handles_ = file_handles_] { std::for_each(file_handles_.cbegin(), file_handles_.cend(), [](const auto file_info) { file_info.first->remove(); }); });
...
void Peer_wire_client::connect_to_peers(const QList<QUrl> & peer_urls) noexcept {
	...
	std::for_each(peer_urls.cbegin(), peer_urls.cend(), [this](const auto & peer_url) {
		if(!active_peers_.contains(peer_url)) {
			...
			connect(socket, &Tcp_socket::connected, this, [this, socket] { on_socket_connected(socket); });
		}
	});
}

[[nodiscard]] std::int32_t Peer_wire_client::piece_size(const std::int32_t piece_idx) const noexcept {
...
[[nodiscard]] bool Peer_wire_client::verify_piece_hash(const QByteArray & received_piece, const std::int32_t piece_idx) const noexcept {
	assert(is_valid_piece_index(piece_idx));
...
[[nodiscard]] Peer_wire_client::Piece_metadata Peer_wire_client::piece_info(const std::int32_t piece_idx, const std::int32_t offset) const noexcept {
	assert(is_valid_piece_index(piece_idx));
	assert(offset >= 0);
...
void Peer_wire_client::verify_existing_pieces() noexcept {
	...
	auto verify_piece = [this](auto verify_piece_callback, const std::int32_t piece_idx) -> void {
		...
		if(piece_idx < total_piece_cnt_) {
			...
			QTimer::singleShot(0, this, [verify_piece_callback, piece_idx] { verify_piece_callback(verify_piece_callback, piece_idx + 1); });
		} else {
	...
	QTimer::singleShot(0, this, [verify_piece] { verify_piece(verify_piece, 0); });
}
...
void Peer_wire_client::on_socket_connected(Tcp_socket * const socket) noexcept {
	...
	connect(this, &Peer_wire_client::piece_verified, socket, [socket](const std::int32_t dled_piece_idx) { socket->send_packet(craft_have_message(dled_piece_idx)); });
...
void Peer_wire_client::on_socket_ready_read(Tcp_socket * const socket) noexcept {
	...
	auto is_valid_socket = [socket = QPointer(socket)] { return socket && socket->state() == Tcp_socket::ConnectedState && socket->bytesAvailable(); };
...
[[nodiscard]] QByteArray Peer_wire_client::craft_allowed_fast_message(const std::int32_t piece_idx) noexcept {
...
[[nodiscard]] QByteArray Peer_wire_client::craft_have_message(const std::int32_t piece_idx) noexcept {
	using util::conversion::convert_to_hex;
...
[[nodiscard]] QByteArray Peer_wire_client::craft_handshake_message() const noexcept {
...
[[nodiscard]] QByteArray Peer_wire_client::craft_piece_message(const QByteArray & piece_data, const std::int32_t piece_idx, const std::int32_t piece_offset) noexcept {
	using util::conversion::convert_to_hex;
...
[[nodiscard]] QByteArray Peer_wire_client::craft_bitfield_message(const QBitArray & bitfield) noexcept {
	using util::conversion::convert_to_hex;
...
[[nodiscard]] std::optional<std::pair<QByteArray, QByteArray>> Peer_wire_client::verify_handshake_reply(Tcp_socket * const socket, const QByteArray & reply) const noexcept {
...
void Peer_wire_client::on_block_request_received(Tcp_socket * const socket, const QByteArray & request) noexcept {
	...
	QTimer::singleShot(0, this, [this, socket = QPointer(socket), send_piece, send_reject_message, requested_piece_idx = requested_piece_idx] {
		...
		if(auto piece = read_from_disk(requested_piece_idx); piece && verify_piece_hash(*piece, requested_piece_idx)) {
			...
			QTimer::singleShot(piece_cleanup_timeout, this, [this, requested_piece_idx] { clear_piece(requested_piece_idx); });
		} else {
			send_reject_message();
		}
	});
...
[[nodiscard]] std::optional<std::pair<qsizetype, qsizetype>> Peer_wire_client::beginning_file_handle_info(const std::int32_t piece_idx) const noexcept {
	assert(is_valid_piece_index(piece_idx));
...
[[nodiscard]] bool Peer_wire_client::write_to_disk(const QByteArray & received_piece, const std::int32_t received_piece_idx) noexcept {
...
[[nodiscard]] std::optional<QByteArray> Peer_wire_client::read_from_disk(const std::int32_t requested_piece_idx) noexcept {
...
[[nodiscard]] bool Peer_wire_client::is_valid_reply(Tcp_socket * const socket, const QByteArray & reply, const Message_Id received_msg_id) noexcept {
...
[[nodiscard]] QSet<std::int32_t> Peer_wire_client::generate_allowed_fast_set(const std::uint32_t peer_ip, const std::int32_t total_piece_cnt) noexcept {
...
void Peer_wire_client::on_piece_downloaded(Piece & dled_piece, const std::int32_t dled_piece_idx) noexcept {
	...
	if(verify_piece_hash(dled_piece.data, dled_piece_idx) && write_to_disk(dled_piece.data, dled_piece_idx)) {
		...
		QTimer::singleShot(std::chrono::seconds(5), this, [this, dled_piece_idx] { clear_piece(dled_piece_idx); });
	} else {
...
[[nodiscard]] util::Packet_metadata Peer_wire_client::extract_packet_metadata(const QByteArray & reply) {
...
template <Peer_wire_client::Message_Id msg_id> [[nodiscard]] QByteArray Peer_wire_client::craft_generic_message(const util::Packet_metadata packet_metadata) noexcept {
...
[[nodiscard]] bool Peer_wire_client::validate_metadata_piece_info(const std::int64_t piece_idx, const std::int64_t received_raw_dict_size) const noexcept {
...
[[nodiscard]] QByteArray Peer_wire_client::craft_metadata_request(const std::int64_t block_idx, const std::int8_t peer_ut_metadata_idx) noexcept {
	assert(block_idx >= 0);
